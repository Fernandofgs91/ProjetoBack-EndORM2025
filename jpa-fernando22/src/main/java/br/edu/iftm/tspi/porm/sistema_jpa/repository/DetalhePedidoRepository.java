...
public interface DetalhePedidoRepository extends JpaRepository<DetalhePedido, DetalhePedidoId> {
    ...
    public List<ProdutoPedidoDTO> buscarPorProduto(@Param("produtoId") Integer produtoId);

    @Query("SELECT new br.edu.iftm.tspi.porm.sistema_jpa.dto.ClienteProdutoTotalDTO(p.produto.codigo, p.produto.nome, SUM(p.quantidade * p.precoVenda)) " +
           "FROM DetalhePedido p WHERE p.pedido.cliente.id = :clienteId GROUP BY p.produto.codigo, p.produto.nome")
    public List<ClienteProdutoTotalDTO> buscarTotalPorProdutoCliente(@Param("clienteId") String clienteId);
    ...
    public List<DetalhePedido> buscarPedidosPorClienteEPeriodo(@Param("clienteId") String clienteId,
                                                        @Param("dataInicio") Date dataInicio,
                                                        @Param("dataFim") Date dataFim);

    @Query("SELECT SUM(p.quantidade * p.precoVenda) FROM DetalhePedido p WHERE p.produto.categoria.id = :categoriaId")
    public Double buscarTotalPorCategoria(@Param("categoriaId") Integer categoriaId);
